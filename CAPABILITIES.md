...
- Date: 2026-06-19
- By: Claude (period-level availability + calendar-sync fix + self-learning roadmap items 1-3)
- Tests: **330 passed, 0 failed** (full backend suite, 2026-06-19). Frontend `tsc --noEmit` + `npm run build` pass.
- Notes: **Weekly availability is now per-period** — crew can block morning/afternoon/evening of each weekday (not just whole days). `UserWeeklyAvailability` gained a `period` column (startup migration expands existing all-day blocks to all three periods); `GET/PUT /users/me/weekly-availability` exchange a `{blocks: {weekday: [periods]}}` map; the admin user-list marks a weekday unavailable only when all three periods are blocked; Settings shows a days×periods grid. Earlier this session: **Google Calendar sync no longer requires crew** (booked+dated jobs auto-sync to the Holy Hauling calendar; crew optional attendees); self-learning roadmap **items 1-3** (outcome layer → retrieval grounding → quote-grounding eval); the escalation overlay. Next roadmap piece: item 4 (regeneration/fine-tune).
