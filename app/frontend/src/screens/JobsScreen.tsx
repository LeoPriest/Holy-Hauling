...
// Optional — set VITE_GOOGLE_MAPS_API_KEY in app/frontend/.env to enable address previews
...
function streetViewUrl(address: string): string | null {
  ...
  return (
    ...
    `?size=640x220&location=${encodeURIComponent(address)}` +
...
function JobCard({ job, userRole, users, patchStatus, addAssignment, removeAssignment }: JobCardProps) {
  ...
  const svUrl = job.job_location ? streetViewUrl(job.job_location) : null
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      ...
      {svUrl && (
        <img
          src={svUrl}
          alt={`Street view of ${job.job_location}`}
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1 min-w-0">
            ...
            {job.job_location ? (
              <a
                href={mapsUrl(job.job_location)}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm text-indigo-600 dark:text-indigo-400 mt-0.5 block hover:underline"
              >
                📍 {job.job_location}
              </a>
            ) : (
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">📍 No location</p>
            )}

            {job.job_date_requested && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">📅 {job.job_date_requested}</p>
            )}
            {job.scope_notes && (
        ...
        {(job.en_route_at || job.started_at) && (
          <div className="flex gap-3 flex-wrap text-xs text-gray-400 dark:text-gray-500 mb-3">
            {job.en_route_at && <span>🚗 {fmtTime(job.en_route_at)}</span>}
        ...
        {job.crew.length > 0 && userRole === 'crew' && (
          <div className="flex gap-1 flex-wrap mb-3">
            {job.crew.map(name => (
              <span key={name} className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 px-2 py-0.5 rounded-full">
                {name}
              </span>
            ))}
          </div>
          ...
          <div className="border-t dark:border-gray-700 pt-3">
            ...
            <div className="flex gap-2 flex-wrap items-center">
              ...
              {job.crew.map(username => {
                ...
                return (
                  <span key={username} className="inline-flex items-center gap-1 text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 px-2 py-0.5 rounded-full">
                    {username}
                    <button
...
export function JobsScreen() {
  ...
  const displayJobs = jobView === 'scheduled'
    ? jobs.filter(j => j.job_phase === null)
    : jobs.filter(j => j.job_phase === 'en_route' || j.job_phase === 'started')

  const inProgressCount = jobs.filter(j => j.job_phase === 'en_route' || j.job_phase === 'started').length
        ...
        {([
          ...
          >
            ...
            {tab.badge != null && tab.badge > 0 && (
              <span className="text-xs bg-amber-500 text-white rounded-full px-1.5 py-0.5 leading-none font-semibold">
                {tab.badge}
              </span>
            )}
          </button>
